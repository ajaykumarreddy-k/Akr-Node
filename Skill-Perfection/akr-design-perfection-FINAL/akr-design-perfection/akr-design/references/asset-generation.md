# Asset Generation — Icons, Favicons, Images, Palettes
...
> **Linux / Cross-Platform Note:** `sips` is macOS-only. On Linux (or any platform without sips),
> use `magick` (ImageMagick) for all operations documented with `sips` below. Quick equivalents:
> - `sips -z H W input.png --out output.png` → `magick input.png -resize WxH! output.png`
> - `sips --resampleWidth 800 input.png --out output.png` → `magick input.png -resize 800 output.png`
> - `sips -s format jpeg input.png --out output.jpg` → `magick input.png output.jpg`
> - `sips -g all input.png` → `magick identify -verbose input.png`
> - `sips --rotate 90 input.png --out output.png` → `magick input.png -rotate 90 output.png`
>
> The bundled `scripts/generate-favicons.sh` and `scripts/generate-app-icons.sh` already handle
> this with an automatic `sips`-first, `magick`-fallback pattern.
