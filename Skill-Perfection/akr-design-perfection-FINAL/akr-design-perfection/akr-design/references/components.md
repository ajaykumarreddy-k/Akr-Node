...
> ⚠️ **TOKEN OVERRIDE RULE (mandatory when used inside AKR Design Perfection):**
> Every code example below uses raw Tailwind values (e.g. `bg-blue-600`, `bg-gray-100`, `hover:text-blue-600`) for readability only.
> In any real AKR build, **all color, spacing, radius, and shadow values MUST be replaced with design tokens from Step 2.6b.3** — e.g. `bg-[var(--color-primary)]`, `text-[var(--fg)]`, `hover:bg-[var(--color-primary-hover)]`.
> Shipping a component that still reads `bg-blue-600` from this file is a token violation and an AI-slop signal.
